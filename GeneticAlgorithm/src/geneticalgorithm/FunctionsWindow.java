...
public class FunctionsWindow {
    ...
    public static void main(String[] args) {
        ...
        GeneticAlgorithm(200, 200, 0, 0.9, k);
        //GeneticAlgorithm(200, 200, 1,0,c2);
    }

    static void GeneticAlgorithm(int maxGen, int popSize, double crossProb, double mutProb, String key) {
        String[] arr = new String[popSize];
        ...
        for (int i = 0; i < arr.length; i++) {
            arr[i] = randomString(50);
        }

        for (int gen = 1; gen <= maxGen; gen++) {
            double average = 0, top = Double.MAX_VALUE;
            for (int i = 0; i < arr.length; i++) {
                double fit = fitness(key, arr[i]);
                average += fit;
                if (fit < top) {
                    top = fit;
                }
                System.out.println("String: " + arr[i] + " Fitness: " + fitness(key, arr[i]) + " At index: " + i);
            }
            System.out.println("Average of the Fitness: " + (average / arr.length) + " Top Fitness: " + top);
            for (int i = 0; i < arr.length; i++) { //Seperates the best values from five random values
                best[i] = tournamentSelection(key, arr[(int) (Math.random() * arr.length)], arr[(int) (Math.random() * arr.length)], arr[(int) (Math.random() * arr.length)], arr[(int) (Math.random() * arr.length)]);
    ...
    static String tournamentSelection(String parent, String... arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            if (best == "") {
                ...
                if (fitness(arr[i], parent) < fit) {
                    fit = fitness(arr[i], parent);
                    best = arr[i];
                }
            }
        }
